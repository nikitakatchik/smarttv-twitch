...
(function (global) {
  ...
  function post(query, onData, onFail) {
    // GraphQL is ALWAYS anonymous. gql.twitch.tv authorizes the public web
    // Client-ID below; it does NOT accept a user OAuth token minted by a
    // different (device-flow) client — sending one gets the whole request 401'd,
    // which breaks browse for every logged-in user. Login lives on Helix only
    // (followed channels); sub-only playback has no public third-party path
    // (see docs/LOGIN.md). Do not attach Authorization here.
