 ...
 * project folder (dist/.tizen-sdk/) so `npm run cert` / `npm run release` can build
 * and sign a .wgt with no system-wide Tizen install.
 *
...
// --ensure: used as a pre-hook of `cert`/`release`. Silent no-op if ANY Tizen CLI
// is already resolvable; otherwise install. Lets those targets depend on the SDK.
